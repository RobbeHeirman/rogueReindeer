...
NioApp = function (NioApp, $, window, document) {
    "use strict";
    var $win = $(window), $doc = $(document), $body = $('body'), $header = $('.header-main');
    var _navBreak = 992, _mobBreak = 768, _mobMenu = 'menu-mobile', _has_fixed = 'has-fixed', _is_shrink = 'is-shrink',
        _block = 'nk-block', _body_class = 'nio-theme', _currentURL = window.location.href,
        _currentHST = window.location.hostname, _headerHT = ($header.innerHeight() - 2),
    ...
    NioApp.Util.preLoader = function () {
        var $preloader = $('.preloader'), $spinner = $('.spinner');
        if ($preloader.exists()) {
    ...
    NioApp.Util.toggler = function () {
        ...
        $doc.on('click', 'body', function (e) {
            ...
            if (!$elm_tog.is(e.target) && $elm_tog.has(e.target).length === 0 && !$elm_tig.is(e.target) && $elm_tig.has(e.target).length === 0) {
    ...
    NioApp.Util.accordionActive = function () {
        var $accordion_item = $('.accordion-item'), $accordion_title = $('.accordion-title');
    ...
    NioApp.MainMenu = function () {
        var $navbar_toggle = $('.navbar-toggle'), $main_navbar = $('.header-navbar'),
            $main_navbar_classic = $('.header-navbar-classic'), $menu_toggle = $('.menu-toggle'),
            $menu_link = $('.menu-link'), _main_menu = '.header-menu', _menu_drop = '.menu-drop',
            _open_nav = 'open-nav', _nav_overlay = '.header-navbar-overlay', _open_menu = 'menu-shown',
        ...
        $('a' + _scroll_tigger + '[href*="#"]:not([href="#"])').on("click", function () {
            if (location.pathname.replace(/^\//, '') === this.pathname.replace(/^\//, '') && location.hostname === this.hostname) {
                ...
                if ($toHash.length) {
                    ...
                    $('html, body').delay(150).animate({scrollTop: ($toHash.offset().top - _headerHT)}, 1000, "easeInOutExpo");
                    return false;
                }
            }
        });
    ...
    NioApp.scrollAct = function () {
        ...
        $body.scrollspy({target: '#' + _header_menu_id, offset: (_headerHT + 2),});
    };
    ...
    NioApp.Plugins.flexslider = function () {
        ...
        if ($FlexSlioder.exists()) {
            $FlexSlioder.each(function () {
                var $self = $(this), f_SlideShowSpeed = ($self.data('slide-speed')) ? $self.data('slide-speed') : 3000,
    ...
    NioApp.Plugins.select2 = function () {
        ...
        if ($select.exists()) {
            $select.each(function () {
                var $self = $(this), _theme = ($self.data('select2-theme')) ? $self.data('select2-theme') : 'bordered',
    ...
    NioApp.Plugins.popup = function () {
        var $content_popup = $('.content-popup'), $video_popup = $('.video-popup'), $image_popup = $('.image-popup');
        var popupInit = {};
        ...
        popupInit.image_popup = function () {
            if ($image_popup.exists()) {
                $image_popup.each(function () {
                    $(this).magnificPopup({type: 'image', mainClass: 'mfp-fade image-popup'});
                });
            }
        };
            ...
            $particles_bg.each(function () {
                ...
                particlesJS($self_id, {
                    "particles": {
                        ...
                        "color": {"value": _pt_base,},
                        "shape": {
                            "type": "circle",
                            "opacity": _pt_shape_op,
                            "stroke": {"width": 0, "color": _pt_shape,},
                            "polygon": {"nb_sides": 5}
                        },
                        "opacity": {
                            "value": _pt_base_op,
                            "random": false,
                            "anim": {"enable": false, "speed": 1, "opacity_min": 0.12, "sync": false}
                        },
                        "size": {
                            "value": 6,
                            "random": true,
        ...
        if ($videobg.exists()) {
            $videobg.each(function () {
                ...
                if ($this.hasClass('bg-video-youtube') && !$this.hasClass('cover-enabled')) {
                    $this.YTPlayer({
                        fitToBackground: true, videoId: video, callback: function () {
                            console.clear();
                        }
                    });
                }
    ...
    NioApp.Plugins.dataTable = function () {
        ...
        if ($dt_filter.exists()) {
            var dt_filter = $dt_filter.DataTable({
                ...
                "language": {"search": '<em class="ti ti-search"></em>', "searchPlaceholder": "Search"},
            });
