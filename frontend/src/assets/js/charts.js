    ...
    NioApp.Chart.ChartJs = function () {
        NioApp.Chart.ChartJs.Doughnut = function (_canvas, _titles, _colors, _amounts, _canvas_border, _canvas_cutout) {
            if ($('#' + _canvas).length) {
                ...
                var doughnut_chart = new Chart(_canvas_el, {
                    ...
                    options: {
                        legend: {display: false, labels: {boxWidth: 10, fontColor: '#000'}},
                        rotation: -2,
        ...
        NioApp.Chart.ChartJs.Pie = function (_canvas, _label, _title, _colors, _colors_hover, _amounts, _canvas_border, _canvas_cutout) {
            if ($('#' + _canvas).length) {
                ...
                var pie_chart = new Chart(_canvas_el, {type: 'pie', data: data_set, options: options_set});
                $win.on('resize', function () {
                    pie_chart.resize();
                });
            }
        ...
        if ($chart_s2.length > 0) {
            $chart_s2.each(function () {
                ...
                if (typeof _canvas !== 'undefined' && _canvas !== '') {
                    ...
                    $chart_data.each(function () {
                        var l = $(this).data('label'), t = $(this).data('title'), sl = $(this).data('subtitle'),
                            c = $(this).data('color'), hc = $(this).data('color-hover'), p = $(this).data('amount');
                        item_label.push(l);
                        item_title.push(t);
                        item_color.push(c);
        ...
        if ($chart_s1.length > 0) {
            $chart_s1.each(function () {
                ...
                if (typeof _canvas !== 'undefined' && _canvas !== '') {
                    ...
                    $chart_data.each(function () {
                        var l = $(this).data('title'), sl = $(this).data('subtitle'), c = $(this).data('color'),
                            hc = $(this).data('color-hover'), p = $(this).data('amount');
                        item_label.push(l);
                        item_color.push(c);
                        item_color_hover.push(hc);
                        item_percent.push(p);
